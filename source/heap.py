...
class MinHeap(object):
    ...
    def _bubble_up(self, index):
        ...
        if item < parent_item: #out of order
            index = parent_index
            self._bubble_up(index)
        elif item >= parent_item: #in order
    ...
    def _bubble_down(self, index):
        ...
        child_index = 0
        if item <= self.items[left_index] and self.items[right_index]:
            return
        if item > self.items[left_index] and self.items[left_index] <= self.items[right_index]:
            child_index = left_index
        elif item > self.items[right_index] and self.items[right_index] <= self.items[left_index]:
